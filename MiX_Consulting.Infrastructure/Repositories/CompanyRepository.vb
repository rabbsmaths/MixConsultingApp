...
Namespace Repositories
    Public Class CompanyRepository
        ...
        ''' </summary>
        Public Function SearchCompanies(filterText As String) As IEnumerable(Of CompanySummaryDTO)
            ...
            Try
                Using conn As New SqlConnection(_connStr)
                    Return conn.Query(Of CompanySummaryDTO)(sql, New With {.Query = "%" & filterText & "%"})
                End Using
        ...
        ''' </summary>
        Public Sub AddCompany(companyName As String, vatNum As String, regNum As String, addressId As Integer)
        ...
        ''' </summary>
        Public Sub UpdateCompany(companyId As Integer, companyName As String, vatNum As String, regNum As String, addressId As Integer)
            ...
            Try
                Using conn As New SqlConnection(_connStr)
                    conn.Open()
                    Using trans = conn.BeginTransaction()
                        ' Added the explicit trans variable pass back to prevent transaction scope leakage
        ...
        Public Function GetAllAddresses() As IEnumerable(Of AddressLookupDTO)
            Const sql As String = "SELECT AddressID, Line1 + ' (' + City + ')' AS InlineDisplay FROM Addresses;"
            Try
                Using conn As New SqlConnection(_connStr)
